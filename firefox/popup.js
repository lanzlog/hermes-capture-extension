...
async function init() {
  const { hermesPort, hermesEnabled } = await browser.storage.local.get([
    "hermesPort",
    "hermesEnabled",
  ]);
...
document.getElementById("enabled").addEventListener("change", async (ev) => {
  ...
  updateEnabledLabel(enabled);
  await browser.storage.local.set({ hermesEnabled: enabled });
...
document.getElementById("save").addEventListener("click", async () => {
  ...
  await browser.storage.local.set({ hermesPort: port, hermesEnabled: enabled });
